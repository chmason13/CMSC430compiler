...
public:
    ...
    u64 hash() const
    {
        ...
        for (u32 i = 0; i < sizeof(hKey); ++i && ++data)
        {
            h = h ^ *data;
            h = h * 0x100000001b3;
        }
    ...
    bool operator==(const hKey& k) const
    {
...
//  Immutable Hash Methods

const hamt<hKey, hKey>* prim_hash_aux(const hamt<hKey, hKey>* h, u64 k, u64 v)
{
    if (k == V_NULL || v == V_NULL) {
        return h;
    } else {
        u64* kp = DECODE_CONS(k);
        u64* vp = DECODE_CONS(v);

        const hKey* const key = new ((hKey*)alloc(sizeof(hKey))) hKey(kp[0]);
        const hKey* const val = new ((hKey*)alloc(sizeof(hKey))) hKey(vp[0]);

        return prim_hash_aux(h->insert(key, val), kp[1], vp[1]);
    }
}

u64 prim_hash(u64 klst, u64 vlst)
{
    const hamt<hKey, hKey>* h = new ((hamt<hKey,hKey>*)alloc(sizeof(hamt<hKey,hKey>))) hamt<hKey,hKey>();
    
    if (klst != V_NULL && vlst != V_NULL) {
        ASSERT_TAG(klst, CONS_TAG, "Tried to make hash with non list keys parameter.")
        ASSERT_TAG(vlst, CONS_TAG, "Tried to make hash with non list values parameter.")

        h = prim_hash_aux(h, klst, vlst);
    }
     
    u64* hp = (u64 *)h;
    u64* hash = (u64*)alloc(sizeof(u64) + sizeof(hamt<hKey,hKey>*));
    hash[0] = (1 << 3) | HASH_OTHERTAG;
    hash[1] = *hp;

    return ENCODE_OTHER(hash);
}
GEN_EXPECT2ARGLIST(applyprim_hash, prim_hash)

u64 prim_hash_ref(u64 h, u64 k) {
    ASSERT_TAG(h, OTHER_TAG, "first argument to hash-ref must be a hash")

    if (k == V_NULL)
        fatal_err("hash-ref must be called with a key argument");

    if ((((u64*)DECODE_OTHER(h))[0]&7) != HASH_OTHERTAG) 
        fatal_err("hash-ref not given a properly formed hash");
    
    const hamt<hKey, hKey>* hash = (hamt<hKey, hKey>*)((u64*)DECODE_OTHER(h))[1];
    const hKey* const key = new ((hKey*)alloc(sizeof(hKey))) hKey(k);
    const hKey* const val = hash->get(key);

    if (val == 0)
        fatal_err("key value pair does not exist for provided key");

    return val->key;
}
 GEN_EXPECT2ARGLIST(applyprim_hash_ref, prim_hash_ref)

u64 prim_hash_set(u64 h, u64 k, u64 v) {
    ASSERT_TAG(h, OTHER_TAG, "first argument to hash-set must be a hash")

    if (k == V_NULL || v == V_NULL)
        fatal_err("hash-set must be called with key and value arguments");

    if ((((u64*)DECODE_OTHER(h))[0]&7) != HASH_OTHERTAG) 
        fatal_err("hash-set not given a properly formed hash");
    
    const hamt<hKey, hKey>* hash = (hamt<hKey, hKey>*)((u64*)DECODE_OTHER(h))[1];
    const hKey* const key = new ((hKey*)alloc(sizeof(hKey))) hKey(k);
    const hKey* const val = new ((hKey*)alloc(sizeof(hKey))) hKey(v);
    hash = hash->insert(key, val);
    u64* hp = (u64 *)hash;
    ((u64*)DECODE_OTHER(h))[1] = *hp;

    return h;
}
GEN_EXPECT3ARGLIST(applyprim_hash_set, prim_hash_set)

u64 prim_hash_remove(u64 h, u64 k) {
    ASSERT_TAG(h, OTHER_TAG, "first argument to hash-remove must be a hash")

    if (k == V_NULL)
        fatal_err("hash-remove must be called with a key argument");
...
GEN_EXPECT2ARGLIST(applyprim_hash_remove, prim_hash_remove)
