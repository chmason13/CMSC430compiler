...
(define (test-compile compile gc top-level-e)  
  (define val1 (eval-top-level top-level-e))   
  (define ll (with-handlers ([exn:fail? (lambda (x) (displayln x))])
               (compile top-level-e gc)))       
  (define val2 (eval-llvm ll))
  (if (equal? val1 val2)
      #t
      (begin
        (display (format "Test-compile: two different values (~a and ~a) before and after top-level processing\n"
                         val1 val2))
        #f)))
...
;(define libgc-path "/usr/local/lib/libgc-dev.a")
...
(define (eval-llvm llvm-str)
  ...
  (system (string-append clang++-path " combined.ll " "-std=c++11 -o bin"))
...
;(define (test-compile compile top-level-e)
                ...
                ;(closure-convert
                 ;(anf-convert
                  ;(alphatize
                   ;(assignment-convert
                    ;(desugar
                     ;(top-level top-level-e))))))))
  ;(display "Top-level value: \n")
  ;(display val0)
  ;(display "LLVM value: \n")
 ...
 ; (define val1 (eval-llvm llvm))
  ...
  ;(if (equal? val0 val1)
   ;   #t
    ;  (begin
     ;   (display (format "Test-compile: two different values (~a and ~a) before and after closure conversion.\n"
      ;                       val0 val1))
       ; #f)))
